# db_V3.py —— 昆岛气象系统数据库模块（SQLite）
# 与 V2 一样，本版本仅文件名更新，逻辑完全兼容 V3
...
def insert_forecast(date_str, wind, temp_min, temp_max, weather):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO forecasts (date, wind, temp_min, temp_max, weather) VALUES (?, ?, ?, ?, ?)",
    ...
    with get_conn() as conn:
        ...
        c.execute(
            ...
            (obs, station, raw,
             wind_dir, wind_speed, wind_gust,
             vis, temp, dew,
             weather_text, rain_flag, rain_level,
             c1, h1, c2, h2, c3, h3)
        )
        conn.commit()
...
def insert_rain_event(time_str, rain_level, rain_code, note):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO rain_events (start_time, rain_level_cn, rain_code, note) VALUES (?, ?, ?, ?)",
